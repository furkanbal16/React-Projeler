...
const TodoItems = () => {
  ...
  if (items.length == 0) {
    return <div className="text-red-600 font-bold text-xl">Liste boş...</div>
  }
  else if (filtered.length == 0 && filterMode == "Active") {
    return <div className="text-red-600 font-bold text-xl">Yapılacak Eylem Bulunamadı</div>
  }
  else if (filtered.length == 0 && filterMode == "Completed") {
    return <div className="text-red-600 font-bold text-xl">Tamamlanmış Eylem Bulunamadı</div>
  }
  return (
    <>
          ...
          return (
            <div key={id} className="flex items-center justify-between">
              <div className="space-x-2 flex items-center w-4/5">
                ...
                <label
                  htmlFor={id}
                  className={`${completed ? "line-through font-medium" : "font-bold"
